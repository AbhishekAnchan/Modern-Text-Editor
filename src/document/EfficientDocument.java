...
public class EfficientDocument extends Document {
	...
	private void processText() {
		...
		if(size != 0 && isWord(tokens.get(size-1))) {
		    numSentences++;
		}
	}
	...
	public static void main(String[] args) {
	    	testCase(new EfficientDocument("This is a test.  How many???  "
	    		...
	    		16, 13, 5, 100.0729);
	    	testCase(new EfficientDocument(""), 0, 0, 0, 0.0);
	    	testCase(new EfficientDocument("sentence, with, lots, of, commas.!  "
	    		+ "(And some poaren)).  The output is: 7.5."), 15, 11, 4, 88.6801);
	    	testCase(new EfficientDocument("many???  Senteeeeeeeeeences are"), 6, 3, 2, 36.1125); 
	    	testCase(new EfficientDocument("Here is a series of test sentences. Your program should "
			+ "find 3 sentences, 33 words, and 49 syllables. Not every word will have "
			+ "the correct amount of syllables (example, for example), "
			+ "but most of them will."), 49, 33, 3, 70.0518);
		testCase(new EfficientDocument("Segue"), 2, 1, 1, 36.62);
		testCase(new EfficientDocument("Sentence"), 2, 1, 1, 36.62);
		testCase(new EfficientDocument("Sentences?!"), 3, 1, 1, -47.98);
		testCase(new EfficientDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, "
			+ "nam dolores explicari forensibus ad."), 32, 15, 1, 11.13);	
	}
}